...
namespace Benchmark
{
    ...
    public class BenchmarkTotalExecutionTimeOfAllPriorityWork
    {
        ...
        public void TestPriorityNotPropagatedThreadStaticSeed()
        {
            ...
            for (int i = 0; i < 200; i++)
            {
                var task1 = _threadStaticPrioritySeedScheduler.Run(
                    async () =>
                    {
                        await DoWorkAsync(cts.Token);
                    },
                    2);
                var task2 = _threadStaticPrioritySeedScheduler.Run(
                    async () =>
                    {
                        await DoWorkAsync(cts.Token);
                    },
                    1);
                var task3 = _threadStaticPrioritySeedScheduler.Run(
                    async () =>
                    {
                        await DoWorkAsync(cts.Token);
                    },
        ...
        public void TestPriorityPropagatedViaPerPriorityLane()
        {
            ...
            for (int i = 0; i < 200; i++)
            {
                var task1 = _perPriorityLaneScheduler.Run(
                    async () =>
                    {
                        await DoWorkAsync(cts.Token);
                    },
                    2);
                var task2 = _perPriorityLaneScheduler.Run(
                    async () =>
                    {
                        await DoWorkAsync(cts.Token);
                    },
                    1);
                var task3 = _perPriorityLaneScheduler.Run(
                    async () =>
                    {
                        await DoWorkAsync(cts.Token);
                    },
        ...
        public void TestPriorityPropagatedViaSyncContext()
        {
            ...
            for (int i = 0; i < 200; i++)
            {
                var task1 = _priorityDispatcherContext.Run(
                    async () =>
                    {
                        await DoWorkAsync(cts.Token);
                    },
                    2);
                var task2 = _priorityDispatcherContext.Run(
                    async () =>
                    {
                        await DoWorkAsync(cts.Token);
                    },
                    1);
                var task3 = _priorityDispatcherContext.Run(
                    async () =>
                    {
                        await DoWorkAsync(cts.Token);
                    },
        ...
        public void TestPowerThreadPool()
        {
            ...
            for (int i = 0; i < 200; i++)
            {
                tasks[i] = _powerPool.QueueWorkItem(
                    ...
                    workOptionLow);
                tasks[i + 200] = _powerPool.QueueWorkItem(
                    async () =>
                    {
                        await DoWorkAsync(cts.Token);
                    },
                    workOptionNormal);
                tasks[i + 400] = _powerPool.QueueWorkItem(
                    async () =>
                    {
                        await DoWorkAsync(cts.Token);
                    },
